...
--현재시퀀스 확인 -> 증가값 수정(가장높은값)-> nextval->증가값1로변경 -> 실행
select dept3_seq.currval from dual;
alter sequence dept3_seq increment by -19;
select dept3_seq.nextval from dual;--다음 시퀀스 값 명령
...
--index
--index는 primary key, unique제약조건에서 자동으로 생성되고, 조회를 빠르게해주는 hint역할을 합니다
--index는 조회를 빠르게 하지만, 무작위하게 많은 인데스를 생성해서 사용하면 오히려, 성능 부하를 일으킬 수 있습니다.
