...
select * from departments;
insert into departments values(290,'DBA',200,1800);
insert into departments(department_id,department_name) values(300,'서버관리자');
select * from departments;
rollback;
...
rollback;
update emps set salary =100000 where employee_id =100; --100번만 수정
...
update emps set (salary, phone_number) = (select salary, phone_number from emps where employee_id = 101) where employee_id = 100;
...
alter table emps add CONSTRAINT emps_manager_fk foreign key (manager_id) REFERENCES emps (employee_id);
...
when matched then--조건에 일치할 경우
    update set a.phone_number = b.phone_number,
                a.salary = b.salary,
                a.commission_pct = b.commission_pct,
                a.department_id = b.department_id
when not matched then--조건이 일치하지 않을 경우(타겟 테이블에 실행)
...
select * from emps_it;
--실습--
delete from emps_it where employee_id = 102;
insert into emps_it(employee_id, first_name, last_name, email, hire_date, job_id) values(102,'렉스','킴','lex','01/04/06','AD_VP');
insert into emps_it(employee_id, first_name, last_name, email, hire_date, job_id) values(103,'니나','커','nina','20/04/06','AD_VP');

--employees테이블을 매번 수정되는 테이블이라고 가정하고,
--기존데이터는 email, phone, salary, manager_id, department_id는 업데이트 되도록처리
--새로유입된 데이터는 그냥 그대로 전부 추가
merge into emps_it a
