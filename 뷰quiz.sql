...
select job_title,avg(salary),sum(salary),count(salary) from view_emp_dept_job group by job_title order by avg(salary);
--뷰 삭제
drop view view_emp;
drop view view_emp_dept_job;
/*
view는 insert,update,delete가 일어나는 경우에 실제테이블에도 반영이 됩니다.
그리고 많은 제약사항이 있습니다. 그냥 원본테이블에DML명령을 사용.
*/
select * from view_emp_dept_job;
select * from view_emp;
desc employees;
--name은 가상열 이라서 insert가 불가능
...
--
insert into view_emp(employee_id, first_name,salary) values(300,'test',100000);
